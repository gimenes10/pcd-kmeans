/* kmeans_1d_omp.c
 * K-means 1D - Versão OpenMP (Paralelizada)
 * Etapa 1 do Projeto
 * 
 * Compilação: gcc -O2 -std=c99 -fopenmp kmeans_1d_omp.c -o kmeans_1d_omp -lm
 * Uso: ./kmeans_1d_omp dados.csv centroides_iniciais.csv [num_threads] [max_iter] [eps]
 */
...
/* Assignment Paralelo: Para cada ponto X[i], encontra cluster c com menor distância */
static double assignment_step_1d_omp(const double *X, const double *C, int *assign,
                                     int N, int K) {
    double sse = 0.0;
    ...
    for (int i = 0; i < N; i++) {
        ...
        // Loop interno não precisa de paralelização (K é pequeno)
        for (int c = 0; c < K; c++) {
            double diff = X[i] - C[c];
            double d = diff * diff;
            if (d < bestd) {
...
/* Update Paralelo: Recalcula centróides usando acumuladores por thread */
static void update_step_1d_omp(const double *X, double *C, const int *assign,
                               int N, int K) {
    int num_threads = omp_get_max_threads();
    
    // Acumuladores locais por thread
    double *sum_local = (double*)calloc((size_t)(num_threads * K), sizeof(double));
    int *cnt_local = (int*)calloc((size_t)(num_threads * K), sizeof(int));
    ...
    // Fase 1: Acumular localmente (cada thread tem seus acumuladores)
    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
        double *my_sum = &sum_local[tid * K];
        int *my_cnt = &cnt_local[tid * K];
    ...
    // Fase 2: Reduzir acumuladores de todas as threads
    ...
    for (int t = 0; t < num_threads; t++) {
        for (int c = 0; c < K; c++) {
            sum_global[c] += sum_local[t * K + c];
            cnt_global[c] += cnt_local[t * K + c];
        }
    }
    
    // Fase 3: Calcular novos centróides
    for (int c = 0; c < K; c++) {
        if (cnt_global[c] > 0) {
            C[c] = sum_global[c] / (double)cnt_global[c];
        } else {
            // Cluster vazio: mantém o centróide
            C[c] = X[0];
        }
    }
...
int main(int argc, char **argv) {
    if (argc < 3) {
        printf("Uso: %s dados.csv centroides_iniciais.csv [num_threads=4] [max_iter=100] [eps=1e-6] [assign.csv] [centroids.csv]\n", argv[0]);
    ...
    kmeans_1d_omp(X, C, assign, N, K, max_iter, eps, &iters, &sse, &sse_history);
    ...
    // Exibir resultados
    printf("========================================\n");
    printf("K-means 1D - Versão OPENMP\n");
    printf("========================================\n");
    printf("Parâmetros:\n");
    printf("  N = %d pontos\n", N);
    printf("  K = %d clusters\n", K);
